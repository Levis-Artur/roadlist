...
function verifyOfficerToken(token: string) {
  const payload = jwt.verify(token, env.jwtSecret) as jwt.JwtPayload & { badgeNumber?: string; department?: string };
  if (!payload.badgeNumber || !payload.department) throw new Error('invalid officer token');
  return payload;
}

async function resolvePhotoActor(request: Request): Promise<{ admin?: AdminTokenPayload; officer?: { badgeNumber: string; department: string } }> {
  ...
  try {
    ...
    try {
      ...
      return { officer: { badgeNumber: String(officer.badgeNumber), department: String(officer.department) } };
    } catch {
...
export async function getPhotoController(request: Request, response: Response, next: NextFunction) {
  try {
    ...
    if (actor.admin?.role === 'REGIONAL_ADMIN' && photo.routeSheet?.department !== actor.admin.department) {
